...
def run_extraction(cfg: dict):
    ...
    members, nonmembers, ft_val = u.load_data_splits(
        dataset_name,
        n_members=n_members,
    ...
    tokenizer, model_ref, model_tgt = u.prepare_tokenizer_and_models(model_name, dataset_name=dataset_name)
    model_tgt = u.apply_lora_if_enabled(model_tgt, lora_cfg)

    print(f"Fine-tuning target model...")
    u.finetune_model_on_texts(
        model_tgt,
        tokenizer,
        members,
        ft_val,
    ...
    mem_feats, mem_masks, mem_token_ids = u.extract_per_token_features_both(
        model_tgt,
        model_ref,
        tokenizer,
        members,
    ...
    non_feats, non_masks, non_token_ids = u.extract_per_token_features_both(
        model_tgt,
        model_ref,
        tokenizer,
        nonmembers,
    ...
    train_size, val_size, test_size = u.save_features(
        output_dir,
        mem_feats,
        mem_masks,
        non_feats,
    ...
    u.save_extraction_metadata(metadata, output_dir)

    manifest_entry = u.ManifestEntry(
        combo_id=combo_id,
        model_name=model_name,
        dataset_name=dataset_name,
    ...
    u.update_manifest(manifest_path, manifest_entry)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
    python extract_features.py --config configs/extract/gpt2_agnews.yaml
    ...
    python extract_features.py \\
        --config configs/extract/base.yaml \\
